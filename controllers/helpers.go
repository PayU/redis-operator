...
// RedisGroups is a map of leaders and their corresponding followers
...
const (
	clusterFailoverCheckInterval     = 3 * time.Second
	clusterFailoverCheckTimeout      = 30 * time.Second
	k8sPodsDeletionCheckInterval     = 2500 * time.Millisecond
	k8sPodsDeletionCheckTimeout      = 30 * time.Second
	...
	clusterFollowerJoinCheckInterval = 1 * time.Second
	clusterFollowerJoinCheckTimeout  = 15 * time.Second
)
...
const (
	...
	// RecoverFollowerNodes means some followers pod/s failed/crashed and has been replaced
	// with a new k8s pods but still didn't join the cluster
	RecoverFollowerNodes RedisClusterState = "RecoverFollowerNodes"
...
func getCurrentClusterState(logger logr.Logger, redisCluster *dbv1.RedisCluster) RedisClusterState {
...
func NewRedisGroups(redisPods *corev1.PodList, redisClusterNodes *rediscli.RedisClusterNodes) *RedisGroups {
	...
	for _, redisPod := range redisPods.Items {
		...
		if redisClusterNodes != nil {
			for _, redisClusterNode := range *redisClusterNodes {
				if !redisClusterNode.IsFailing() && redisClusterNode.Leader == "-" && strings.HasPrefix(redisClusterNode.Addr, redisPod.Status.PodIP) {
					group.LeaderID = redisClusterNode.ID
				}
			}
		}
...
func (r *RedisClusterReconciler) getRedisClusterPods(ctx context.Context, redisCluster *dbv1.RedisCluster, podType string) (*corev1.PodList, error) {
	pods := &corev1.PodList{}
	...
	err := r.List(ctx, pods, client.InNamespace(redisCluster.ObjectMeta.Namespace), client.MatchingLabels(matchingLabels))
	if err != nil {
		return nil, err
	}
...
// createFollowersForLeader create all followers pods for a specific leader pod
// the function return the number of created followers for the specific leader
func (r *RedisClusterReconciler) createFollowersForLeader(ctx context.Context, redisCluster *dbv1.RedisCluster, leaderNumber int, startingSequentialNumber int) (int, error) {
	followersCount := int(redisCluster.Spec.LeaderFollowersCount)

	for i := 0; i < followersCount; i++ {
		followerPod, err := r.NewFollowerPod(redisCluster, leaderNumber, startingSequentialNumber+i)
		if err != nil {
			return 0, err
		}

		if err = r.Create(ctx, &followerPod, applyOpts...); err != nil {
			if !strings.Contains(err.Error(), "already exists") {
				return 0, err
			}

			r.Log.Info(fmt.Sprintf("follower-%d-%d already exists (redis-pod-%d)", leaderNumber, i, startingSequentialNumber+i))
		}
	}
...
func (r *RedisClusterReconciler) createLeaders(ctx context.Context, redisCluster *dbv1.RedisCluster, nodeCount int) error {
	for i := 0; i < nodeCount; i++ {
		leaderPod, err := r.NewLeaderPod(redisCluster, i, i)
		if err != nil {
			return err
		}

		r.Log.Info(fmt.Sprintf("deploying leader-%d", i))
		if err = r.Create(ctx, &leaderPod, applyOpts...); err != nil {
			if !strings.Contains(err.Error(), "already exists") {
				return err
			}
...
func (r *RedisClusterReconciler) createNewCluster(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	r.Log.Info("creating new cluster")
	desiredLeaders := int(redisCluster.Spec.LeaderReplicas)

	// create config map
	configMap, err := r.NewRedisSettingsConfigMap(redisCluster)
	...
	// create service
	service, err := r.NewService(redisCluster)
	...
	headlessService, err := r.NewHeadlessService(redisCluster)
	...
	err = r.createLeaders(ctx, redisCluster, desiredLeaders)
	if err != nil {
		return err
	}

	redisCluster.Status.ClusterState = string(DeployingLeaders)

	return nil
}

func (r *RedisClusterReconciler) checkRedisNodes(ctx context.Context, redisCluster *dbv1.RedisCluster, nodeRole string) (bool, error) {
	podsReady := true
	pods, err := r.getRedisClusterPods(ctx, redisCluster, nodeRole)
	if err != nil {
		return false, err
	}
...
func (r *RedisClusterReconciler) handleDeployingLeaders(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	r.Log.Info("handling deploying leaders")

	leadersReady, err := r.checkRedisNodes(ctx, redisCluster, "leader")
	if err != nil {
		return err
	}

	if leadersReady {
		redisCluster.Status.ClusterState = string(InitializingLeaders)
	}

	return nil
}

func (r *RedisClusterReconciler) handleDeployingFollowers(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	r.Log.Info("handling deploying followers")

	followersReady, err := r.checkRedisNodes(ctx, redisCluster, "follower")
	if err != nil {
		return err
	}
...
func (r *RedisClusterReconciler) handleInitializingLeaders(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	r.Log.Info("handling initializing leaders")

	leaderPods, err := r.getRedisClusterPods(ctx, redisCluster, "leader")
	if err != nil {
		return err
	}
	...
	redisCluster.Status.ClusterState = string(ClusteringLeaders)

	return nil
}

func (r *RedisClusterReconciler) handleInitializingFollowers(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
...
func (r *RedisClusterReconciler) handleClusteringLeaders(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	...
	for _, leaderPod := range leaderPods.Items {
		...
		createdFollowers, err := r.createFollowersForLeader(ctx, redisCluster, leaderNumber, startingfollowerSequentialNumber)
		if err != nil {
			return err
		}
	...
	redisCluster.Status.ClusterState = string(DeployingFollowers)

	return nil
}

func (r *RedisClusterReconciler) handleClusteringFollowers(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	...
	redisCluster.Status.TotalExpectedPods = redisCluster.Spec.LeaderReplicas * (redisCluster.Spec.LeaderFollowersCount + 1)
...
func (r *RedisClusterReconciler) handleLeaderNodesRecoverState(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
		...
		for _, k8sRedisPod := range k8sRedisPods.Items {
			if strings.HasPrefix(redisNode.Addr, k8sRedisPod.Status.PodIP) {
				if k8sRedisPod.Labels["redis-node-role"] == "follower" {
					...
					var currentPodDeployed corev1.Pod

					if err = r.Get(ctx, types.NamespacedName{
						Namespace: k8sRedisPod.Namespace,
						Name:      k8sRedisPod.Name,
					}, &currentPodDeployed); err != nil {
						return err
					}

					valuesToPatch := `{ "metadata": { "labels": { "redis-node-role": "leader" } } }`
					err = r.Patch(ctx, &currentPodDeployed, client.RawPatch(types.StrategicMergePatchType, []byte(valuesToPatch)))
					...
					err = wait.PollImmediate(k8sPatchCheckInterval, k8sPatchCheckTimeout, func() (bool, error) {
						...
						if currentPodDeployed.Labels["redis-node-role"] != "leader" {
							return false, nil
						}
	...
	redisCluster.Status.ClusterState = string(RecoverFollowerNodes)
...
func (r *RedisClusterReconciler) handleFollowerNodesRecoverState(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	...
	for _, group := range redisGroups {
		groupLeaderID, err := r.RedisCLI.GetMyClusterID(group.LeaderPod.Status.PodIP)
		if err != nil {
			return err
		}

		leaderReplicas, err := r.RedisCLI.GetLeaderReplicas(group.LeaderPod.Status.PodIP, groupLeaderID)
		if err != nil {
			return err
		}

		currentLeaderReplicaCount := leaderReplicas.Count
		r.Log.Info(fmt.Sprintf("current leader [%s] replica count is %d", groupLeaderID, currentLeaderReplicaCount))
		...
		for _, followerPod := range group.FollowerPods {
			...
			if shouldJoin {
				r.RedisCLI.AddFollower(followerPod.Status.PodIP, group.LeaderPod.Status.PodIP, groupLeaderID)

				// wait for follower to join
				err = wait.PollImmediate(clusterFollowerJoinCheckInterval, clusterFollowerJoinCheckTimeout, func() (bool, error) {
					var err error
					r.Log.Info("waiting for follower to join the cluster")
					...
					if leaderReplicas.Count == currentLeaderReplicaCount+1 {
						currentLeaderReplicaCount++
						return true, nil
					}
...
func (r *RedisClusterReconciler) handleClusterReadyState(ctx context.Context, redisCluster *dbv1.RedisCluster) error {
	...
	// are unreachable so the cluster will considered them in failure state and master fail over will occur
	...
	// do to all the verifications and fix any issues if found.
...
func (r *RedisClusterReconciler) forgetFailingNodes(ctx context.Context, redisCluster *dbv1.RedisCluster,
	...
	if len(failingNodes) == 0 {
		r.Log.Info("no failing nodes was found in the cluster")
		return false, nil
	}
	...
	// failing redis nodes can occur from a crashed pod or from internal error.
	// in case of internal error the failing redis node pod is still exists (but may not be healthy).
	// in this case we need to make sure to remove the failing pod so we can deploy a new one instead
	...
	if len(podsToDelete) > 0 {
		...
		err = wait.PollImmediate(k8sPodsDeletionCheckInterval, k8sPodsDeletionCheckTimeout, func() (bool, error) {
			var err error
			...
			if len(k8sRedisPods.Items) > int(redisCluster.Status.TotalExpectedPods)-len(podsToDelete) {
				return false, nil
			}
		...
		// we need to fetch the update k8s pods list again
		k8sRedisPods, err = r.getRedisClusterPods(ctx, redisCluster, "any")
		if err != nil {
			return true, err
		}
...
func (r *RedisClusterReconciler) replaceK8sFailedNodes(ctx context.Context, redisCluster *dbv1.RedisCluster,
	...
	err = wait.PollImmediate(clusterFailoverCheckInterval, clusterFailoverCheckTimeout, func() (bool, error) {
		r.Log.Info("waiting for master failover process to finish..")
		var clusterInfo *rediscli.RedisClusterNodes
		var currentLeaderNumber int32
		var err error
		...
		if currentLeaderNumber < redisCluster.Spec.LeaderReplicas {
			return false, nil
		}
...
// for example the pod list is [redis-node-0 redis-node-1 redis-node-2 redis-node 4] and the expected is 5
