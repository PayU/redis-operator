			...
			for name, id := range failingForgets {
				...
				if exists && node != nil {
					...
					if err != nil && strings.Contains(err.Error(), "Can't forget my master") {
						isMaster, err := r.checkIfMaster(node.Ip)
						if err == nil && isMaster {
							for name, _ := range healthyNodes {
								if _, failing := failingForgets[name]; !failing {
									if h, exists := v.Nodes[name]; exists {
										r.reshardAndKeepInMap(node.Name, node.LeaderName, h.Ip, h.Id, v)
										break
									}
								}
							}
...
func (r *RedisClusterReconciler) recoverNodes(redisCluster *dbv1.RedisCluster, v *view.RedisClusterView) bool {
	...
	for _, n := range r.RedisClusterStateView.Nodes {
		...
		if n.NodeState == view.DeleteNode || n.NodeState == view.DeleteNodeKeepInMap || n.NodeState == view.ReshardNode || n.NodeState == view.ReshardNodeKeepInMap {
			continue
		}
		wg.Add(1)
		go func(n *view.NodeStateView) {
